...
- `src/utils/` — pure/domain logic, one concern per file:
  - Loading: `formats.ts` is the single list of openable geometry formats (extensions, loader kind, Aide table, pickers' `accept`, drop-zone hint) - `electron/main.js` keeps its own copy (dialog filters, `GEOMETRY_FILE_RE`) and `package.json` the file associations: keep all three in sync. `stepLoader.ts` (spawns `src/workers/step.worker.js`, which runs occt-import-js off the main thread) reads STEP, IGES and BREP (`format` picks `ReadStepFile` / `ReadIgesFile` / `ReadBrepFile`); IGES usually arrives as loose trimmed surfaces, so `surfaceSewing.ts` regroups faces into bodies with consistent, outward winding (brepFaces keep the original CAD faces). `threeDxmlLoader.ts` reads 3DXML (zip + XML, TESSELLATED representation only; instance matrices baked in). `dxfLoader.ts` reads ASCII DXF (LINE, (LW)POLYLINE with bulges, CIRCLE, ARC, ELLIPSE, SPLINE, 3DFACE, INSERT of blocks; $INSUNITS -> mm) as thin flat ribbons whose centerline vertices are the exact drawing coordinates (the app is mesh-based). `stlLoader.ts`, `objLoader.ts`, `brepEdges.ts`, `surfacePatches.ts`
  - Scene/camera: `cameraFit.ts` is the ONE whole-model framing (`frameGoal`: model fills 75 % of the tighter side of the view, aimed at its apparent centre, near/far from its size; `gridForBounds`: classic grid, cells ~1/25 of the model on 1-2-5 mm steps, thick line every 5 cells, ~3x the model) used by file opening (iso), Réinitialiser la vue / État d'origine, the VUES presets, Zoom ajusté and the PDF 4-view capture. Its `modelBounds` only counts part meshes (`userData.nodeId`), never `Box3.setFromObject(model)`: helpers parented to the model (the hidden exploded-view guide lines sit at the origin) stretched the box, so a file modelled far from the origin opened tiny. `cameraViews.ts`, `animateCamera.ts` (orbiting slerp move, duration from the angle), `cameraNavigation.ts` (Zoom fenêtre goal that keeps the current orientation), `displayMode.ts`, `colorPalette.ts`, `themeColors.ts`. Navigation buttons after VUES (Toolbar `NAV_TOOLS`): store `navMode` ('select' | 'pan' | 'rotate' | 'zoom') remaps OrbitControls' LEFT button in `Viewer3D.tsx` (right-drag pan and wheel zoom never change) and only 'select' lets a click select or an empty click clear the selection; `zoomWindowMode` is one-shot and shares the box-select rectangle overlay; `zoomToFit` / `zoomToRect` are registered by the Scene like `goToView`. Shortcut Z = zoom ajusté (F is already the front view).
...
- Launch screen: `components/SplashScreen.tsx` + `utils/splashAngle.ts` (explicit angle profile: still 0.2 s, half-sine ramp, 240 deg/s, symmetric stop, exactly 360 deg at 2.0 s then 0). `public/logo.png` is never modified: the big gear turns from 3 layers in `public/splash/` built from it (`gear-base` = logo with what lies behind the gear, `gear-ring` = the gear alone, `gear-front` = inner mechanism, bearing, caliper stem and jaw tip, all original pixels; only teeth hidden under the bearing/stem are cloned from neighbouring teeth). Before 0.2 s and from 2.0 s the ORIGINAL logo is drawn (pixel-identical). Integer device-pixel scale (no blur), prefers-reduced-motion = still logo. Dev-only: `?splash-hold` keeps it on screen and `window.__pindiSplash.renderAt(t)` freezes a frame.
...
- **SOLIDWORKS bridge (desktop only)**: proprietary formats (`SOLIDWORKS_BRIDGE_FORMATS` in `formats.ts`, `SOLIDWORKS_FILE_RE` in `main.js`) are sent by path to `electron/solidworks-convert.ps1` (IPC `convert-with-solidworks`, one at a time, 10 min timeout), which opens them read-only in the running SOLIDWORKS and exports a STEP (parts/assemblies) or DXF (drawings) copy that goes through the normal loaders. Windows PowerShell's COM adapter fails on every SOLIDWORKS call (TYPE_E_ELEMENTNOTFOUND) - the script only uses `[System.__ComObject].InvokeMember` late binding, unwraps PSObject arguments (else DISP_E_TYPEMISMATCH), and uses `ModelDoc2.SaveAs3`. It never closes a document the user already had open, emits ASCII error codes only (French text in `SOLIDWORKS_ERRORS`), and rejects an export without geometry (`EMPTY_EXPORT`: after an import stalled - an Inventor .iam - SOLIDWORKS exported empty STEPs, then crashed; restart it). The script ships unpacked (`build.asarUnpack`). Not registered as file associations on purpose (a .SLDPRT double-click must keep opening SOLIDWORKS). `preload.js` buffers `open-file` paths that arrive before React registers its handler.
